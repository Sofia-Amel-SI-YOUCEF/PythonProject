...
url = "https://127.6.5.1.5000/SK_ID_CURR"

# st.write('# Scoring Model')
# st.title('Loan Dashboard')
# st.header('Sofia Amel SI YOUCEF')
...
def st_buttons(client_id):
    ...
    with st.sidebar:
        ...
        b_dist = st.button("Characteristics comparison: id_" + str(client_id))
        b_loc_importance = st.button("Decision details: id_" + str(client_id))
...
def plot_l_importance(df, model, features, b_loc_importance, client_id, num_features):
    """
    ...
    if df[df["SK_ID_CURR"] == client_id].empty:
        ...
        idx = df[df["SK_ID_CURR"] == client_id].index
